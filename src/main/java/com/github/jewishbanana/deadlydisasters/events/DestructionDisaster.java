	...
	public void createTimedStart(int delaySeconds, Vector offset, Player p) {
		...
		plugin.getServer().getScheduler().runTaskLater(plugin, new Runnable() {
			public void run() {
				...
				// UPDATED: Exclude the new purge-style disasters from the default broadcast
				// since they handle their own custom broadcasting logic!
				if ((boolean) wo.settings.get("event_broadcast") && type != Disaster.BANDITRAID && type != Disaster.INFESTATION && type != Disaster.DEATHPARADE)
					Utils.broadcastEvent(level, "destructive", type, temp, p);
