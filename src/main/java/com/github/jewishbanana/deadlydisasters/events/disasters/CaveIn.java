...
class CaveInBlock {
	...
	// 1. Only allow natural cave blocks to cave in (no stone bricks, no concrete, etc.)
	private static final java.util.Set<Material> CAVE_MATERIALS = java.util.EnumSet.of(
	...
	// 2. The Support Beam Radius Checker
	private boolean hasSupport(Block center) {
		// Checks a 5x5x5 area around the falling block for support structures
		for (int x = -2; x <= 2; x++) {
			for (int y = -2; y <= 2; y++) {
				for (int z = -2; z <= 2; z++) {
					Material m = center.getRelative(x, y, z).getType();
					// If there is a fence, log, plank, or wall nearby, it holds the ceiling up!
	...
	public void fall(BlockData material, double speed) {
		if (depth > 0) {
			...
			// 3. Apply our new checks: Is it a cave block? Is it unsupported? Is it outside WorldGuard?
			if (CAVE_MATERIALS.contains(b.getType()) && !hasSupport(b) && !Utils.isZoneProtected(loc)) {
				if (material == null)
				...
				if (spawnLoc.getBlock().isPassable()) {
					FallingBlock fb = b.getWorld().spawnFallingBlock(spawnLoc, material);
					
					fb.setHurtEntities(false); 
					fb.setDropItem(false);
					fb.setVelocity(new Vector(0, speed, 0));
					fb.setMetadata("dd-fb", new org.bukkit.metadata.FixedMetadataValue(classInstance.plugin, "protected"));
					classInstance.fallingRoof.add(fb.getUniqueId());
					
					b.getWorld().spawnParticle(org.bukkit.Particle.BLOCK_DUST, spawnLoc, 15, 0.5, 0.2, 0.5, material);
				}
			} else {
				// If it's supported or an artificial block, skip it safely
